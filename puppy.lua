	...
	for i, v in pairs(self.flags) do
		if v.current ~= nil then
			if typeof(v.current) == "Color3" then
				cfg[i] = {"Color3", v.current.R, v.current.G, v.current.B}
			elseif typeof(v.current) == "EnumItem" then
				cfg[i] = {"EnumItem", tostring(v.current.EnumType), v.current.Name}
			else
				cfg[i] = v.current
			end
		end
...
function library:loadconfig(cfg)
	local success, decoded = pcall(function() return hs:JSONDecode(readfile(cfg)) end)
	if not success or type(decoded) ~= "table" then return end
	
	for i, v in pairs(decoded) do
		if self.flags[i] and self.flags[i].set then
			if type(v) == "table" and v[1] == "Color3" then
				pcall(function() self.flags[i]:set(Color3.new(v[2], v[3], v[4])) end)
			elseif type(v) == "table" and v[1] == "EnumItem" then
				pcall(function() self.flags[i]:set(Enum[v[2]][v[3]]) end)
			else
				pcall(function() self.flags[i]:set(v) end)
			end
		end
	end
end
...
function sections:toggle(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = toggle
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = toggle
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
...
function sections:slider(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = slider
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = slider
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
...
function sections:dropdown(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = dropdown
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = dropdown
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
...
function sections:buttonbox(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = buttonbox
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = buttonbox
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
...
function sections:multibox(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = multibox
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = multibox
		end
	end
	--
	self.library.labels[#self.library.labels+1] = value
...
function sections:textbox(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = textbox
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = textbox
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
...
function sections:keybind(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = keybind
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = keybind
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
...
function sections:colorpicker(props)
	...
	if pointer then
		...
		local lib = self.library or self
		if lib and lib.flags then
			lib.flags[tostring(pointer)] = colorpicker
		end
		if lib and lib.pointers then
			lib.pointers[tostring(pointer)] = colorpicker
		end
	end
	--
	self.library.labels[#self.library.labels+1] = title
